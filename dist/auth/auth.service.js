...
let AuthService = class AuthService {
    ...
    async validateUser(userDto) {
        ...
        if (!user) {
            throw new common_1.UnauthorizedException({ message: 'Пользователя с таким логином или email нет!' });
        }
    ...
    async regisration(userDto) {
        ...
        if (candidate) {
            throw new common_1.HttpException('Пользователь с таким именем существует', common_1.HttpStatus.BAD_REQUEST);
        }
        const hashPass = await bcrypt.hash(userDto.pass, 5);
        const user = await this.userService.createUser(Object.assign(Object.assign({}, userDto), { pass: hashPass }));
